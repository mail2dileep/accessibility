    ...
    if (typeof violations !== 'undefined') {
        ...
        if (violationCount > 0) {
            for (i = 0; i < violationCount; i += 1) {
                ...
                if (typeof nodes !== 'undefined') {
                    outputRow += url.replace(/,/g , '-') + ',' + violation.id.replace(/,/g , '-') + ',' + violation.impact.replace(/,/g , '-') + ',' + violation.helpUrl.replace(/,/g , '-')+violation.desription.replace(/,/g , '"');
                    outputRowPrefix = outputRow;
                    nodeCount = nodes.length;
    ...
    if (typeof violations !== 'undefined') {
        ...
        if (violationCount > 0) {
            for (i = 0; i < violationCount; i += 1) {
                ...
                if (typeof nodes !== 'undefined') {
                    outputRow += url.replace(/,/g , '-') + ',' + violation.id.replace(/,/g , '-') + ',' + violation.impact.replace(/,/g , '-') + ',' + violation.helpUrl.replace(/,/g , '-')+ violation.id.description(/"/g , '');
                    outputRowPrefix = outputRow;
                    nodeCount = nodes.length;
...
exports.processResults = function (results, fileType, fileName, createNewReport,flag) {
    var any,
        anyCount,
        anys,
        delimiter,
    ...
    if (createNewReport || flag == 0) {
        outputRow = ('URL' + delimiter + 'Rule'+delimiter +'Violation description' + delimiter + 'Violation Type' + delimiter +'Impact'
                    + delimiter + 'LEVEL'+ delimiter + 'HTML Element' + delimiter + 'Messages' + delimiter +  'DOM Element\r');
    ...
    if (typeof violations !== 'undefined') {
        ...
        if (violationCount > 0) {
            for (i = 0; i < violationCount; i += 1) {
                ...
                if (typeof nodes !== 'undefined') {
                    outputRow += url.replace(/,/g , '-') + delimiter + violation.help.replace(/,/g,'-')+delimiter+ violation.description.replace(/"/g,'') + delimiter + violation.id.replace(/,/g , '-') + delimiter + violation.impact.replace(/,/g , '-')

                    outputRowPrefix = outputRow;
                    nodeCount = nodes.length;
					wcagCount=wcag_tags.length;
					for (k = 0;k<wcagCount;k+=1) {
						wcagtag=wcag_tags[k];
						if (wcagtag == 'wcag2a' || wcagtag == 'wcag2aa' || wcagtag == 'best-practice' || wcagtag == 'wcag2aaa') {
							
							outputRow = outputRowPrefix;
							outputRow+=delimiter+wcagtag;
							}
